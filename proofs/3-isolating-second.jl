...
# TODO: Change this
norms = [
	MethodOfParticularSolutions.norm(domains[i], us[i], λs[i], numpoints = 100) 
...
active_boundaries = [MethodOfParticularSolutions.active_boundaries(domains[i], us[i]) for i in eachindex(domains)]
...
begin
	μs = [sqrt(MethodOfParticularSolutions.area(domains[i])) * max_values[i] / norms[i] for i in eachindex(domains)]
